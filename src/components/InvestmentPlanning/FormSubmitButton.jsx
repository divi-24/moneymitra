const SubmitButtonInvestiMate = ({ onClick, disabled }) => {
    return (
        <>
        <a
            className={`relative inline-flex items-center justify-start px-6 py-3 overflow-hidden font-medium transition-all bg-pink-500 rounded-xl group ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
